...
# ---------------------------------------------------------------------------
# Path resolution — wyrd/src/ is 2 levels from repo root
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Lazy imports for AIOS core types (avoid hard circular dependency)
# These types live in platform/executive-daemon/src/ which is a sibling tree.
# When running tests from wyrd/, the caller must add executive-daemon/src to PYTHONPATH.
# ---------------------------------------------------------------------------

def _get_attention_state_class():
    from attention_manager import AttentionState
    return AttentionState

def _get_tracked_item_class():
    from attention_manager import TrackedItem
    return TrackedItem

def _get_commitment_class():
...
class ProjectStore:
    ...
    def list_as_tracked_items(self) -> list:
        """
        ...
        """
        AttentionState = _get_attention_state_class()
        TrackedItem = _get_tracked_item_class()
...
class CommitmentStore:
    ...
    def list_as_rules_engine_commitments(self) -> list:
        """Return active commitments as rules_engine.Commitment NamedTuples."""
        Commitment = _get_commitment_class()
        return [
